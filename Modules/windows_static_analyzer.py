...
class WindowsAnalyzer:
    def __init__(self, target_file, enable_mitre, json_mode=False):
        self.target_file = target_file
        self.json_mode = json_mode # Store the mode
        ...
        # Handle File Type Detection Silently
        ...
        if self.is_dotnet:
            self.log(f"{infoS} File Type: [bold green].NET Executable[white]\n")
            self.dotnet_file_analyzer()
            if not self.json_mode: sys.exit(0) # Don't exit early in JSON mode if you need the final dump
        elif "MSI Installer" in self.exec_type_str:
            self.log(f"{infoS} File Type: [bold green]Microsoft Software Installer[white]\n")
            self.msi_file_analyzer()
            if not self.json_mode: sys.exit(0)
        else:
            self.log(f"{infoS} File Type: [bold green]Windows Executable[white]\n")
    ...
    def dictcateg_parser(self):
        """Modified to update winrep and suppress tables in JSON mode."""
        for key in dictCateg:
            if dictCateg[key] != []:
                if not self.json_mode:
                    if key in ["Keyboard/Keylogging", "Evasion/Bypassing", "System/Persistence", "Cryptography", "Information Gathering"]:
        ...
        for tactic in self.mitre_data_windows:
            ...
            for technique, payload in self.mitre_data_windows[tactic].items():
                ...
                tactic_rows.append(
                    {
                        "technique": technique,
                        "tid": t_id, # Store TID for the table
                        "score": score,
            ...
            mitre_table.add_column("[bold yellow]ID", justify="center") # New Column
            ...
            for row in tactic_rows:
                ...
                # Add the TID to the row
                mitre_table.add_row(
                    row["technique"], 
                    f"[white]{row['tid']}[/white]", 
                    str(row["score"]), 
                    sample_apis
                )
            # ----------------------------
    ...
    def dll_files(self):
        try:
            ...
            for items in self.binaryfile.DIRECTORY_ENTRY_IMPORT:
                ...
                # Always collect data
                if dlStr not in winrep["linked_dll"]:
                    winrep["linked_dll"].append(dlStr)
                
                # Only build table if not JSON
                if not self.json_mode:
                    dllTable.add_row(f"{dlStr}", style="bold red")

            if not self.json_mode:
                self.log(f"\n{infoS} Found {len(winrep['linked_dll'])} Linked DLLs:")
                print(dllTable)

        except Exception:
            if self.json_mode:
                # Ensure a list exists for errors if you want to track them
                if "errors" not in winrep: winrep["errors"] = []
                winrep["errors"].append("Could not parse DLL imports")
            pass
    ...
    def statistics_method(self):
        ...
        self.winrep["filename"] = self.target_file
...
def main():
    if len(sys.argv) < 2:
        err_exit("Usage: windows_static_analyzer.py <file> [report] [mitre] [json]")
    
    fileName = sys.argv[1]
    report_enabled = _arg_is_true(get_argv(2, "False"))
    mitre_enabled = _arg_is_true(get_argv(3, "False"))
    # Handle the new 4th argument for JSON
    json_mode = _arg_is_true(get_argv(4, "False"))
    
    # 1. Execute Analysis
    windows_analyzer = WindowsAnalyzer(target_file=str(fileName), enable_mitre=mitre_enabled, json_mode=json_mode)
    
    # Silence these steps if JSON is enabled to keep the output "clean"
    ...
    # Yara and Sections
    yara_rule_scanner(windows_analyzer.rule_path, fileName, windows_analyzer.winrep)
    windows_analyzer.section_parser()
    windows_analyzer.statistics_method()

    # 2. Decision Point: JSON vs Human
    if json_mode:
        import json
        # We dump the 'winrep' dictionary which contains all the collected data
        print(json.dumps(windows_analyzer.winrep))
    else:
        # If not JSON, we do the normal report printing/saving
        if report_enabled:
            save_report("windows", windows_analyzer.winrep)
        # Any other final 'human' print statements go here...
