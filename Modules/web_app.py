...
def build_summary(report_data: Optional[dict]) -> List[dict]:
    ...
    if "ml_conf" in report_data:
        summary.append({
            "label": "AI Risk Score", 
            "value": f"{float(report_data['ml_conf']) * 100:.0f}/100"
        })

    # Added Target Type (e.g., Win32)
    ...
    if "behavior_score" in report_data:
        summary.append({"label": "Behavior Score", "value": f"{report_data['behavior_score']:.4f}"})
    
    if "packer_score" in report_data:
        summary.append({"label": "Packer/Obfuscation", "value": f"{report_data['packer_score']:.4f}"})

    # Severity logic for Final Risk
    if "final_risk" in report_data:
        risk_val = float(report_data.get("final_risk", 0))
        # Logic: 0 is Green, 70+ is Red
        sev = "low" if risk_val == 0 else ("high" if risk_val >= 70 else "medium")
        
        summary.append({
            "label": "Final Risk", 
            "value": "TRUSTED (0.00/100)" if risk_val == 0 else f"{risk_val:.2f}/100",
            "severity": sev 
        })
...
# Behavioral reports store the label flat; standard reports nest it
    vt_label = (
       str(report_data.get("vt_threat_label") or "").strip()   # behavioral path
       or str(vt_data.get("threat_label") or "").strip()        # standard path
    )
    cluster_family = str(report_data.get("family") or "")

    if vt_label:
        family_display = f"{vt_label}  ({cluster_family})"
    else:
        family_display = cluster_family

    summary.append({
            "label": "Malware Family",
            "value": _fmt_value(family_display),
    })
    summary.append(
        {
            "label": "Filename",
...
def build_frontend_payload(report_data: Optional[dict]) -> dict:
    if not isinstance(report_data, dict):
        return {
            ...
            or str((report_data.get("virustotal_file") or {}).get("threat_label") or "").strip()
            ),
            "vt_section": {
                "available": False,
                "summary": [],
    ...
    "sections", "mitre_attack", "notes", "family" 
    }
    ai_output = ""
    ai_iocs: List[dict] = []
    ...
    all_detailed_panels = _build_detailed_panels(report_data, skip_keys={"carved_executables", "mitre_attack"}) + pcap_panels

    return {
        "summary": build_summary(report_data),
        "ml_raw": int(float(report_data.get("ml_conf", 0)) * 100),
        "final_risk_val": int(min(100, max(0, float(report_data.get("final_risk", 0))))),
...
def execute_preset(sample_path: Path, preset: AnalysisPreset, enable_ai: bool) -> dict:
    ...
    vt_enabled_presets = {("--analyze",), ("--docs",), ("--archive",), ("--behavioral",)}
    ...
    # Also snapshot behavioral reports before running
    before_behavioral = {
        p: p.stat().st_mtime_ns
        for p in REPORTS_ROOT.glob("sc0pe_behavior_report*.json")
        if p.exists()
    }
    started = time.perf_counter()
    ...
    # --- BEHAVIORAL FALLBACK: detect sc0pe_behavior_report_TIMESTAMP.json ---
    if preset.args == ("--behavioral",) and not stored_report_path:
        new_behavioral = [
            p for p in REPORTS_ROOT.glob("sc0pe_behavior_report*.json")
            if p.exists() and (
                p not in before_behavioral
                or p.stat().st_mtime_ns > before_behavioral[p]
            )
        ]
        if new_behavioral:
            newest = max(new_behavioral, key=lambda p: p.stat().st_mtime_ns)
            stored_report_path = _store_generated_report(newest, bucket="main")
    # -------------------------------------------------------------------------
    ...
    # Only embed VT into non-behavioral reports (preserves behavioral core findings)
    if preset.args in vt_enabled_presets and preset.args != ("--behavioral",) and stored_report_path and isinstance(vt_result, dict):
        _embed_vt_result_into_main_report(stored_report_path, vt_result)

    # For behavioral, extract just the threat label to pass back
    vt_threat_label = ""
    if preset.args == ("--behavioral",) and isinstance(vt_result, dict):
        vt_payload = vt_result.get("report_data") or {}
        vt_threat_label = str(vt_payload.get("threat_label") or "").strip()
    ...
    return {
        ...
        "vt_threat_label": vt_threat_label,  # <-- new field
    }
    ...
    while True:
        ...
        try:
            ...
            with JOBS_LOCK:
                ...
                job["vt_threat_label"] = str(result.get("vt_threat_label") or "") # <-- replaces the whole block
