...
auto_extract = len(sys.argv) > 2 and sys.argv[2].strip().lower() == "true"
...
class SignatureChecker:
    def __init__(self, target_file):
        ...
        if self.target_file_size < 52428800:
            # FIX: Single open() call; getbins is kept for seek-based carving,
            # getbins_buffer holds the full bytes — no anonymous handle leak.
            self.getbins = open(self.target_file, "rb")
            self.getbins_buffer = self.getbins.read()
            self.getbins.seek(0)
        else:
        ...
        for off in self.offset_array:
            ...
            try:
                ...
                if carve_size:
                    ...
                    pfile = pf.PE(data=data_to_trim)
            except Exception:
                continue

            # FIX: Use 'with' so the dump file is always closed, even on error.
            try:
                buffer_to_write = pfile.trim()
                with open(f"qu1cksc0pe_carved-{off}.bin", "wb") as dumpfile:
    ...
    def signature_checker(self):
        ...
        # FIX: Use 'with' to ensure the JSON file handle is always closed.
        with open(f"{sc0pe_path}{path_seperator}Systems{path_seperator}Multiple{path_seperator}file_sigs.json") as fsigs_fh:
            fsigs = json.load(fsigs_fh)
        ...
        # Windows side
        if mz_offsets:
            if auto_extract or user_confirm(f"{infoC} Do you want to extract executable files from target file[Y/n]?: "):
    ...
    def search_possible_corrupt_mz_headers(self):
        ...
        # FIX: Accumulate all fixes onto the same buffer instead of overwriting
        # new_buffer on each iteration — previously only the last fix was saved.
        corrupted = 0
        new_buffer = self.getbins_buffer
        for offset in mz_offsets:
            if new_buffer[offset+2:offset+4] != b"\x90\x00":
                print(f"[bold magenta]>>>[white] Possible corrupted MZ header at: [bold green]{hex(offset)}[white]. Attempting to fix that!")
                new_buffer = new_buffer[:offset+2] + b"\x90\x00" + new_buffer[offset+4:]
                corrupted += 1
    ...
    def pumped_file_carver(self):
        ...
        # FIX: Close mmfl inside the 'with' block before 'target' is closed,
        # and remove the now-redundant target.close() call after the block.
                ...
                if detected_executables:
                    ...
                    for binary in detected_executables:
                        ...
                        try:
                            ...
                            continue
                        # FIX: Use 'with' so the dump file is always closed.
                        try:
                            buffer_to_write = pfile.trim()
                            ...
                            with open(f"qu1cksc0pe_carved-{binary[0]}.bin", "wb") as dumpfile:
                                dumpfile.write(buffer_to_write)
